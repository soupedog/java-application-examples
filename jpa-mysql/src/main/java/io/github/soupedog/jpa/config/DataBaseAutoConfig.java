...
@EnableJpaRepositories(
        ...
        // 默认开启事务，"JpaRepository" 实现类继承而来的事务配置导致它提供的普通查询方法也在事务之中执行，需类似主动 "@Transactional(Transactional.TxType.NEVER)" 来进行事务禁用
        // 或默认关闭事务，但 "JpaRepository" 实现类继承而来语法糖方法需要自己手动添加 "@Transactional")
...
public class DataBaseAutoConfig implements HyggeAutoConfiguration {
    ...
    public LocalContainerEntityManagerFactoryBean entityManagerFactoryBean(@Qualifier("webAppDataSource") HikariDataSource hikariDataSource) {
        ...
        // 设置数据库方言 (可选项参见 "org.hibernate.dialect" 这个包下的类)
